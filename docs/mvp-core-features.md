...
| TURN credentials moved server-side (#538) | Done | Replaced `NEXT_PUBLIC_TURN_*` env vars with server-side `TURN_URL`/`TURN_SECRET`; new `GET /api/turn-credentials` generates ephemeral HMAC-based credentials (RFC 8489); clients fetch via `fetchIceServers()` helper |
