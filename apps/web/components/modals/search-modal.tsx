...
import type { ChannelDocRow, ChannelTaskRow, MessageWithAuthor } from "@/types/database"
...
export function SearchModal({ serverId, onClose, onJumpToMessage }: Props) {
  ...
  const [taskResults, setTaskResults] = useState<ChannelTaskRow[]>([])
  const [docResults, setDocResults] = useState<ChannelDocRow[]>([])
  ...
  const search = useCallback(
    async (q: string, off = 0) => {
      ...
      setLoading(true)
      try {
        const res = await fetch(`/api/search?q=${encodeURIComponent(q)}&serverId=${serverId}&limit=20&offset=${off}`)
        if (!res.ok) return
  ...
  const hasWorkspaceResults = taskResults.length > 0 || docResults.length > 0
  const hasMessageResults = results.length > 0
        ...
        <div className="flex-1 overflow-y-auto">
          {!query.trim() ? (
            <div className="flex flex-col items-center justify-center py-12 gap-2">
              ...
              <p className="text-sm" style={{ color: "#949ba4" }}>Search messages, tasks, and docs across this server</p>
            </div>
          ) : !loading && !hasMessageResults && !hasWorkspaceResults ? (
            <div className="flex flex-col items-center justify-center py-12 gap-2">
              ...
              {results.map((msg) => {
                ...
                return (
                  <button
                    key={msg.id}
                    onClick={() => {
                      onJumpToMessage?.(msg.channel_id, msg.id)
                      onClose()
                    }}
                  ...
                  >
                    <Avatar className="w-8 h-8 flex-shrink-0 mt-0.5">
                      ...
                      <AvatarFallback style={{ background: "#5865f2", color: "white", fontSize: "11px" }}>{initials}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm truncate" style={{ color: "#b5bac1" }}>{msg.content}</p>
                    </div>
                  </button>
                )
              })}

              {hasMessageResults && results.length < total && (
                <button
