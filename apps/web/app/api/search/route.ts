...
export async function GET(req: NextRequest) {
  ...
  if (serverId) {
    const { data: member } = await supabase.from("server_members").select("user_id").eq("server_id", serverId).eq("user_id", user.id).single()
  ...
  if (serverId && !channelId) {
    const { data: channels } = await supabase.from("channels").select("id").eq("server_id", serverId).in("type", ["text", "announcement", "forum", "media"])
  ...
  if (q) messageQuery = messageQuery.textSearch("search_vector", q, { type: "websearch", config: "english" })
  ...
  const [{ data: results, error }, { data: tasks }, { data: docs }] = await Promise.all([
    messageQuery,
    q
      ? supabase
          .from("channel_tasks")
  ...
  return NextResponse.json({ results: results ?? [], total: results?.length ?? 0, tasks: tasks ?? [], docs: docs ?? [] })
}
