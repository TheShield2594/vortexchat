...
--
-- All statements use CONCURRENTLY so they run without acquiring a full table
-- lock in production. CONCURRENTLY cannot run inside a transaction block;
-- Supabase migrations run each file outside an implicit transaction when the
-- file does not start with BEGIN, which is the case here.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_channel_permissions_channel_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_channel_permissions_role_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_member_roles_role_id
...
-- previously indexed.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_dm_channel_members_dm_channel_id
